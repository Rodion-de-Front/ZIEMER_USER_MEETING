    ...
    <>
      ...
      <section className="stats" aria-label={copy.aria}>
        ...
        <div>
          ...
          <strong>{copy.inPerson}</strong>
        </div>
      </section>
      <section aria-labelledby="sections-title">
        <div className="section-heading">
          <div>
            <p className="eyebrow">{copy.nav}</p>
            <h2 id="sections-title">{copy.title}</h2>
          </div>
        ...
        <nav className="tiles" aria-label={copy.navAria}>
          {sections.map(({ id, title, caption, icon: Icon }, index) => (
            <Link
              className={`tile ${tileThemes[index % tileThemes.length]}`}
              to={`/${id}`}
              key={id}
            >
